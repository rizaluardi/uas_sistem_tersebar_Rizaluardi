...
#Membuat string generator untuk message dan thread
...
def messages():
	...
	if request.args.get('thread_id'):
		...
		#memeriksa kepemilikan keamanan Thread
		...
		#Membuat sketsa pesan yang tidak dihapus.
		...
		#menghandler paginasi dengan menggunakan ajax untuk request, pesan, dan penarikan pesan
		...
		if request.args.get('fetch'): #Cek database agar tidak terjadi error pada flask
		...
		#Menandai semua pesan di dalam thread yang berada di current_user yang belum dibaca menjadi sudah dibaca
		...
		#Menetapkan id penerima pada balasan sehingga bila pesan dikirimkan ke sendiri maka tetap bisa jalan.
		...
		#Memberi tahu soket jika semua pesan dibaca untuk menyinkronkan pemberitahuan kotak surat.
		...
		#Memberi tahu soket ketika utas dibaca sehingga halaman pesan dapat memperbarui baca/belum dibaca .
		...
		#mengurutkan setiap utas pesan dengan benar sesuai dengan tanggal waktu pesan yang terakhir diterima.
		unread_ids = {}
		...
		#Ini merupakan memperbaiki utas pesan yang dilihat di /messages/ sehingga pesan duplikat tidak ditampilkan
		thread_list = []
		message_thread_list = []
		for message in unread_messages:
